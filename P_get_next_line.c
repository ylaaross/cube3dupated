...
/*   Updated: 2023/12/20 17:07:56 by ylaaross         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	rest[BUFFER_SIZEG];
	char		buffer[BUFFER_SIZEG + 1];
	int			sread;
	char		*line;

	sread = 1;
	if (fd < 0 || BUFFER_SIZEG <= 0 || fd == 2 || fd == 1)
		return (0);
	initialise(buffer, BUFFER_SIZEG + 1);
	line = ft_calloc(1, 1);
	line = ft_strjoin(line, rest);
	while (sread > 0 && check(line))
	{
		sread = read(fd, buffer, BUFFER_SIZEG);
		if (sread == -1 || (sread == 0 && *line == 0))
		{
			initialise(rest, BUFFER_SIZEG);
			free(line);
			return (0);
		}
		buffer[sread] = 0;
	...
	return (devide (line, rest, BUFFER_SIZEG));
}
